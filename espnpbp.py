...
time = [] #empty list to add info in 
...
df['Quarter'] = np.where(
              df['Detail'].str.contains('Quarter'), df['Detail'].str.contains('Quarter').cumsum(), np.nan) #finds where 'detail' contains the word quarter and displays sum of occurences, from stackoverflow
df['Quarter'] = df['Quarter'].bfill() #backfills previous rows with number from "end of _ quarter" row
